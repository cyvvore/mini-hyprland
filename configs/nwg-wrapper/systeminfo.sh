...
# Get number of available updates using yay (Arch Linux, reads once per boot)
UPDATE_CACHE="/tmp/yay_update_cache_boot"

if [[ ! -f "$UPDATE_CACHE" ]]; then
  if command -v yay &>/dev/null; then
    OFFICIAL=$(yay -Qu | wc -l)
    AUR=$(yay -Qua | wc -l)
    echo "$OFFICIAL" > "$UPDATE_CACHE"
    echo "$AUR" >> "$UPDATE_CACHE"
  else
    echo "0" > "$UPDATE_CACHE"
    echo "0" >> "$UPDATE_CACHE"
  fi
...
read OFFICIAL_UPDATES < "$UPDATE_CACHE"
read -r AUR_UPDATES < <(sed -n '2p' "$UPDATE_CACHE")
